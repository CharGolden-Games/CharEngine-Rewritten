...
class BaseSettingsState extends MusicBeatSubstate
{
    ...
    public function new()
    {
        ...
        {
			var optionText:Alphabet = new Alphabet(290, 260, options[i].name, false);
			...
			{
				var checkbox:Checkbox = new Checkbox(optionText.x - 105, optionText.y, Std.string(options[i].getValue()) == 'true');
			...
			else
			{
				...
				//optionText.xAdd -= 80;
				var valueText:AttachedText = new AttachedText('' + options[i].getValue(), optionText.width + 60);
				valueText.sprTracker = optionText;
				valueText.copyAlpha = true;
				valueText.ID = i;
				grpTexts.add(valueText);
				options[i].child = valueText;
			}
			updateTextFrom(options[i]);
        }

        if (watermark != null)
        {
            var wText:Alphabet = new Alphabet(0, 0, watermark, true);
            wText.setScale(0.3);
            wText.alpha = 0.4;
            add(wText);
            wText.scrollFactor.set();
            wText.y = FlxG.height - (wText.height + 5);
        }
	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
			...
			if(controls.RESET)
			{
				var leOption:Option = options[curSelected];
	...
	function changeSelection(change:Int = 0)
	{
		...
		curOption = options[curSelected]; //shorter lol
		FlxG.sound.play(Paths.sound('scrollMenu'));
	}

	function reloadCheckboxes()
		for (checkbox in grpCheckboxes)
			checkbox.value = Std.string(options[checkbox.ID].getValue()) == 'true'; //Do not take off the Std.string() from this, it will break a thing in Mod Settings Menu
}